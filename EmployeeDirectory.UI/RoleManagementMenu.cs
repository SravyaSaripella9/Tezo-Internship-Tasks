...
namespace EmployeeDirectory.UI
{
    class RoleManagementMenu : IRoleManagementMenu
    {
        ...
        public void GetRoleDetails()
        {
            ...
            Role role = new Role(((Guid.NewGuid()).ToString()), name ?? "", department.Id, description, location.Id);
            rolesController.AddRole(role);
        }
        public void DisplayRoles()
        {
            List<RoleViewModel> roleViewModels = rolesController.GetRoleViewModels();
            if (roleViewModels.Count == 0)
                Console.WriteLine("Sorry, there are no roles to be displayed");
            else
            {
                foreach (RoleViewModel roleViewModel in roleViewModels)
                {
                    Console.WriteLine($"Role Name : {roleViewModel.Name}");
                    Console.WriteLine($"Role Department : {roleViewModel.DepartmentName}");
                    Console.WriteLine($"Role Description : {roleViewModel.Description}");
                    Console.WriteLine($"Role Location : {roleViewModel.LocationName}");
                    Console.WriteLine("========================================");
                }
            }
        }
